...
const int MAXSERVER = 8000, MAXADMIN = 100, MAXLINK = 1e5;
    ...
    // map ehash;
    // should be
    std::map<std::pair<int, int>, std::pair<int, int> > ehash;
    ...
    int main()
    {
        ...
        // "S": "Numer of Servers"
        // "L": "Number of Links"
        // "A": "Number of Network Administrators"
        // "T": "Number of Transformations"
        // why did zzz transform SLAT TO NMCT?
        scanf("%d%d%d%d", &N, &M, &C, &T);

        // pre-create array of nodes of count M (L)
        // WHAT IS FRX?
        for (i = 0; i < M; ++i)
            frx[i] = new node();
        ehash.clear();
        ...
        while (T--)
        {
            ...
            if (cmd == 1)
            {
                ...
                if (!x)
                {
                    //not found in edges storage 
                    printf("Wrong link\n"); 
                    continue; 
                    } 
                    --x; 
                    if (x == i) {
                        //this cable is already controlled by this admin 
                        printf("Already controlled link\n"); 
                        continue; 
                        } 
                        if (table[a][i].count() == 2 || table[b][i].count() == 2) {
                        //at least one of the serves has 2 cables related the new admin 
                        printf("Server overload\n"); 
                        continue; 
                        } 
                        if (table[a][i].count() > 0 && table[b][i].count() > 0 && connected(a, b, i)) {
                            //it is possible to get from one of them to the other 
                            printf("Network redundancy\n"); 
                            continue; 
                        }
                        px = data.second;
                        //get the proper node (i.e. the node for edge a <-> b)
                ...
                else
                {
                    ...
                    {
                        if (al.count() == 1 && bl.count() == 1)
                        { //happy case :)
                            px = al.get();
                            make_first(px);
                            x = px->sum;
                        }
                        else if (al.count() + bl.count() == 3)
                        {
                            //one is extreme, we need to find the next one
                            if (al.count() > 1)
                                swap(al, bl);
                        ...
                        {
                            //hard case, invoking creativity :)
                            px = al.a, qx = al.b, tx = bl.a, ux = bl.b;
                            rpx = xrank(px), rqx = xrank(qx), rtx = xrank(tx), rux = xrank(ux);
                            if (rpx > rqx)
                                swap(px, qx), swap(rpx, rqx);
                            if (rtx > rux)
                                swap(tx, ux), swap(rtx, rux);

                            if (rpx > rtx)
                                swap(px, tx), swap(rpx, rtx), swap(qx, ux), swap(rqx, rux);
                            /**
                 * At this point:
